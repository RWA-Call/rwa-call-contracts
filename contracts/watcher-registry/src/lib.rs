...
impl WatcherRegistry {
    ...
    /// # Errors
    /// Returns [`ContractError::AlreadyInitialized`] if the registry has
    ...
    /// existing admin.
    /// # Panics
    /// Panics if the internal index is inconsistent (should never happen in
    /// practice).
    /// # Errors
    /// Returns [`ContractError::Unauthorized`] if `caller`/`admin` is not a
    /// registered admin.
    ...
    /// # Errors
    /// Returns [`ContractError::Unauthorized`] if `caller`/`admin` is not a
    /// registered admin. Returns [`ContractError::LastAdmin`] if this would
    /// leave the registry with zero admins.
    pub fn remove_admin(
        env: Env,
        caller: Address,
    ...
    /// practice).
    /// # Errors
    /// Returns [`ContractError::Unauthorized`] if `admin` is not a registered
    /// admin. Returns [`ContractError::WatcherNotFound`] if `old_watcher` is
    ...
    ///
    /// # Panics
    /// Panics if the internal watcher index is inconsistent (should never happen in practice).
    #[must_use]
    ...
    /// # Errors
    /// Returns [`ContractError::NotInitialized`] if the registry has not been
    /// initialized yet.
